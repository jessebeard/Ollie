...
export class VaultUI {
    ...
    getVaultSize() {
        ...
        // Cache size computation against the immutable vault reference to prevent
        // expensive synchronous JSON serialization on every UI render (e.g. search).
        if (this._vaultSizeCache && this._vaultSizeCache.vault === this.vault) {
            return this._vaultSizeCache.size;
        }

        const json = JSON.stringify(this.vault.toJSON());
        const size = new TextEncoder().encode(json).length;

        this._vaultSizeCache = { vault: this.vault, size };
        return size;
    }
