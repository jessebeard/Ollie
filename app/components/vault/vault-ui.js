...
export class VaultUI {
    ...
    getVaultSize() {
        ...
        // Cache the vault size to avoid expensive JSON.stringify and TextEncoder operations on every UI update
        const cacheKey = `${this.vault.metadata?.modified}-${this.vault.entries?.length}`;
        if (this._lastVaultSizeCacheKey === cacheKey) {
            return this._lastVaultSizeCache;
        }

        const json = JSON.stringify(this.vault.toJSON());
        const size = new TextEncoder().encode(json).length;

        this._lastVaultSizeCacheKey = cacheKey;
        this._lastVaultSizeCache = size;
