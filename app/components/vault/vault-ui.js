...
export class VaultUI {
    ...
    getVaultSize() {
        ...
        // Bolt: Optimization to cache the vault size, as JSON.stringify + TextEncoder on the whole vault is expensive
        const cacheKey = `${this.vault.metadata?.modified}-${this.vault.entries?.length}`;
        if (this._vaultSizeCache?.key === cacheKey) {
        ...
        this._vaultSizeCache = { key: cacheKey, size };
        return size;
    }
