...
export class VaultUI {
    constructor() {
        ...
        this.vaultSizeCache = new WeakMap();
    ...
    getVaultSize() {
        ...
        if (this.vaultSizeCache.has(this.vault)) {
            return this.vaultSizeCache.get(this.vault);
        }

        // Cache expensive derivations against the immutable object reference to avoid main-thread blocking
        const json = JSON.stringify(this.vault.toJSON());
        const size = new TextEncoder().encode(json).length;

        this.vaultSizeCache.set(this.vault, size);
        return size;
    }
