...
export class VaultUI {
    ...
    getVaultSize() {
        ...
        // Use composite key for cache invalidation based on modified timestamp and entry count
        const cacheKey = `${this.vault.metadata?.modified}-${this.vault.entries?.length}`;
        if (this._vaultSizeCacheKey === cacheKey && this._vaultSizeCache !== undefined) {
