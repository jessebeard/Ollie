...
export class VaultUI {
    constructor() {
        ...
        // ⚡ Bolt: Cache properties for getVaultSize optimization
        this._cachedVaultSize = 0;
        this._cachedVaultKey = null;
    ...
    getVaultSize() {
        ...
        // ⚡ Bolt: Cache expensive derived value using composite key of modified date and entry count
        const cacheKey = `${this.vault.metadata?.modified}-${this.vault.entries?.length}`;
        if (this._cachedVaultKey === cacheKey) {
