...
export class VaultUI {
    ...
    getVaultSize() {
        ...
        // Cache the expensive TextEncoder/JSON.stringify calculation
        // Vaults are effectively immutable; use a composite key to safely detect state changes
        const cacheKey = `${this.vault.metadata?.modified}-${this.vault.entries?.length}`;

        if (this._lastVaultSizeKey === cacheKey) {
        ...
        this._lastVaultSizeKey = cacheKey;
