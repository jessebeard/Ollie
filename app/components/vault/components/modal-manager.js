    ...
    async showForm(title, fields, initialValues = {}) {
        return new Promise((resolve) => {
            ...
            const inputsHtml = fields.map(field => {
                ...
                const nameEscaped = this.escapeHtml(field.name);

                if (type === 'textarea') {
                    return `
                        <div class="form-group">
                            <label>${label}</label>
                            <textarea name="${nameEscaped}" class="form-control" ${required}>${val}</textarea>
                        </div>`;
                }
                ...
                return `
                    <div class="form-group">
                        <label>${label}</label>
                        <input type="${type}" name="${nameEscaped}" value="${val}" class="form-control" ${required}>
                    </div>`;
            }).join('');
