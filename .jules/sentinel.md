...
## 2024-11-20 - XSS Vulnerability via unvalidated window.open URL
**Vulnerability:** Direct use of user-provided URLs in `window.open()` allowed XSS via malicious schemes like `javascript:`.
**Learning:** URLs must be validated before being passed to `window.open()`. Malicious URLs can contain control characters to bypass simple checks.
**Prevention:** Always strip control characters and explicitly reject dangerous protocols using the `URL` constructor before passing a URL to a sink.
