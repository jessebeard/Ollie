...
## 2024-11-20 - URL Handling XSS
**Vulnerability:** The application allowed arbitrary URLs (such as `javascript:`, `vbscript:`, or `data:`) to be passed to `window.open` via the launch URL button.
**Learning:** Functions like `window.open` or `<a href>` can execute malicious scripts when passed dangerous URL protocols.
**Prevention:** Always sanitize URLs by explicitly rejecting dangerous protocols before injecting them into the DOM or calling `window.open`.
