...
## 2024-11-20 - ModalManager innerHTML XSS injection
**Vulnerability:** Unescaped strings were directly passed to `innerHTML` when creating modals with `showForm()`, `prompt()`, `confirm()`, and `showAlert()`, allowing XSS injections from parameters like `initialValues`, `title`, and `message` that could be manipulated through UI flows.
**Learning:** All inputs, even "trusted" internal values (like entry names reused as modal titles or default field values), must be sanitized before rendering into `innerHTML` or string templates.
**Prevention:** Introduce a centralized, safe `escape(str)` function inside `ModalManager` that implements the principles from the previous learning and pass all template inputs through it.
