...
## 2024-11-20 - XSS via unvalidated URL protocol in window.open
**Vulnerability:** XSS vulnerability when opening URLs using `window.open` without sanitizing the protocol (`javascript:`, `data:`, `vbscript:`).
**Learning:** The URL protocol must always be explicitly validated when opening links to prevent XSS. Attackers can obfuscate protocols using control characters and spaces.
**Prevention:** Always sanitize URLs robustly: strip non-printable control characters, then use the `URL` constructor to explicitly reject dangerous `.protocol` values. Apply aggressive space-stripping to a separate copy of the URL solely for protocol validation to avoid corrupting valid URLs.
