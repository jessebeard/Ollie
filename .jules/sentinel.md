...
## 2024-12-07 - XSS in Vault View Launch URL
**Vulnerability:** Untrusted URLs were directly passed into `window.open` in Vault View component, allowing javascript: scheme execution (XSS).
**Learning:** `window.open` and `<a href>` attributes are sinks for DOM-based XSS when provided with malicious protocols (`javascript:`, `data:`, `vbscript:`).
**Prevention:** Implement strict URL sanitization checking for and blocking unsafe schemes before passing user-provided URLs to `window.open`.
