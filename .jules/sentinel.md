...
## 2024-06-13 - Attribute Injection XSS in Legacy Vault UI
**Vulnerability:** XSS via unescaped attribute injection (`data-id="${entry.id}"`) in the legacy `app/vault.js` `renderPasswordCard` method.
**Learning:** Even internal properties like UUIDs (`entry.id`) must be strictly escaped when interpolated into HTML attributes, as the value could be manipulated if loaded from an external/malicious source.
**Prevention:** Always escape all interpolated variables inside HTML strings, including properties within data attributes, using explicit character-level replacement.
