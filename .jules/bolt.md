...
## 2024-07-02 - Array Filtering Optimization
**Learning:** Using `.toLowerCase().includes()` inside a `.filter()` loop causes repeated string allocations and degrades performance during high-frequency UI updates like vault searches.
**Action:** Precompile a case-insensitive `RegExp` (escaping special characters) before the loop and use `regex.test()` instead for significantly faster text matching.
