...
## 2024-06-18 - RegExp optimization for vault search
**Learning:** In high-frequency UI updates like vault searches, repeated string allocations (e.g., `.toLowerCase().includes()`) inside `.filter()` loops are very expensive and O(N) over all entries.
**Action:** Use a single precompiled, case-insensitive `RegExp` initialized before the loop for significantly faster text matching. Also ensure to add a short-circuit null check for the regex to prevent TypeError when the query is empty.
