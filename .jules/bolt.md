...
## 2024-05-15 - Case-insensitive text matching performance
**Learning:** In high-frequency UI updates like vault searches, repeated string allocations (`.toLowerCase().includes()`) inside `.filter()` loops cause performance bottlenecks.
**Action:** Use a single precompiled, case-insensitive `RegExp` (with a short-circuit null check for empty queries) for significantly faster O(N) text matching.
