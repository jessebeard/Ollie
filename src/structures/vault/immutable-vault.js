...
export class PasswordVault {
    ...
    search(query, tags = []) {
        // Bolt: Precompile a case-insensitive RegExp instead of allocating lower-cased strings in the loop
        // Escape special regex characters to prevent SyntaxError on literal queries
        const searchRegex = query ? new RegExp(query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') : null;

        return this.entries.filter(e => {
            const matchesText = !searchRegex || (
                // RegExp.test() coerces null to "null", so ensure the property exists and is truthy first
