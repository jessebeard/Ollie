...
export class PasswordVault {
    ...
    search(query, tags = []) {
        if (!query && (!tags || tags.length === 0)) return [...this.entries];
        // ⚡ Bolt Optimization: Use precompiled RegExp for case-insensitive search to prevent repeated string allocations
        let regex = null;
        if (query) {
